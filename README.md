...
> Only **SM-G985F** is boot-tested. Every other model is built with **its own Samsung defconfig**
> on the shared **G985F driver kernel** — its DTS is in the repo but the flashable keeps the phone's own
> DTB, and the other drops' driver differences (mm, Mali GPU, NPU) are **not** imported (see
> `PROVENANCE.md`) — and has **not** been booted on hardware.
>
...
Config is layered: the mapped Samsung base defconfig (`build.sh` maps each `-m` key to its Samsung name — e.g. `g985f`→`exynos9830-y2slte_defconfig`) + `arch/arm64/configs/ksu.config`
