...
void susfs_add_sus_path(void __user **user_info) {
	...
	if (strstr(info.target_pathname, android_data_path.target_pathname)) {
		...
		new_list->target_ino = inode->i_ino;
		...
		new_list->i_uid = inode->i_uid.val;
		...
		SUSFS_LOGI("target_ino: '%lu', target_pathname: '%s', i_uid: '%u', is successfully added to LH_SUS_PATH_ANDROID_DATA\n",
					new_list->target_ino, new_list->target_pathname, new_list->i_uid);
		info.err = 0;
		goto out_path_put_path;
...
bool susfs_is_sus_android_data_d_name_found(const char *d_name) {
	...
	list_for_each_entry(cursor, &LH_SUS_PATH_ANDROID_DATA, list) {
		...
		if (!strncmp(d_name, cursor->info.target_pathname, cursor->path_len) &&
			...
			is_i_uid_in_android_data_not_allowed(cursor->i_uid))
		{
...
/* sus_net_port: hide a running server's listening TCP/TCP6 port. Re-registered
...
void susfs_add_sus_net_port(void __user **user_info) {
	...
	struct st_susfs_sus_net_port_hlist *cursor = NULL, *temp = NULL, *new_entry = NULL;

	if (copy_from_user(&info, (struct st_susfs_sus_net_port __user*)*user_info, sizeof(info))) {
		info.err = -EFAULT;
		goto out_copy_to_user;
	}
	spin_lock(&susfs_spin_lock_sus_net_port);
	list_for_each_entry_safe(cursor, temp, &LH_SUS_NET_PORT, list) {
		if (cursor->info.target_uid == info.target_uid &&
		    cursor->info.port == info.port) {
			spin_unlock(&susfs_spin_lock_sus_net_port);
			info.err = 0;
			goto out_copy_to_user;
		}
	}
	...
	new_entry = kzalloc(sizeof(struct st_susfs_sus_net_port_hlist), GFP_KERNEL);
	if (!new_entry) {
		info.err = -ENOMEM;
		goto out_copy_to_user;
	}
	memcpy(&new_entry->info, &info, sizeof(struct st_susfs_sus_net_port));
	INIT_LIST_HEAD(&new_entry->list);
	spin_lock(&susfs_spin_lock_sus_net_port);
	list_add_tail(&new_entry->list, &LH_SUS_NET_PORT);
	spin_unlock(&susfs_spin_lock_sus_net_port);
	info.err = 0;
	SUSFS_LOGI("added sus_net_port uid: %u, port: %u\n", info.target_uid, info.port);
out_copy_to_user:
	if (copy_to_user(&((struct st_susfs_sus_net_port __user*)*user_info)->err, &info.err, sizeof(info.err))) {
		info.err = -EFAULT;
	}
}
...
void susfs_del_sus_net_port(void __user **user_info) {
	struct st_susfs_sus_net_port info = {0};
	struct st_susfs_sus_net_port_hlist *cursor = NULL, *temp = NULL;
...
bool susfs_is_sus_net_port(unsigned int uid, unsigned int port) {
	struct st_susfs_sus_net_port_hlist *cursor = NULL;
	bool found = false;

	spin_lock(&susfs_spin_lock_sus_net_port);
	list_for_each_entry(cursor, &LH_SUS_NET_PORT, list) {
		if (cursor->info.target_uid == uid && cursor->info.port == port) {
			found = true;
			break;
		}
	}
...
/* sus_net_unix: hide a running server's abstract unix socket by inode. */
static DEFINE_SPINLOCK(susfs_spin_lock_sus_net_unix);
static LIST_HEAD(LH_SUS_NET_UNIX);

void susfs_add_sus_net_unix(void __user **user_info) {
	struct st_susfs_sus_net_unix info = {0};
	struct st_susfs_sus_net_unix_hlist *cursor = NULL, *temp = NULL, *new_entry = NULL;

	if (copy_from_user(&info, (struct st_susfs_sus_net_unix __user*)*user_info, sizeof(info))) {
		info.err = -EFAULT;
		goto out_copy_to_user;
	}
	spin_lock(&susfs_spin_lock_sus_net_unix);
	list_for_each_entry_safe(cursor, temp, &LH_SUS_NET_UNIX, list) {
		if (cursor->info.target_uid == info.target_uid &&
		    cursor->info.inode == info.inode) {
			spin_unlock(&susfs_spin_lock_sus_net_unix);
			info.err = 0;
			goto out_copy_to_user;
		}
	}
	...
	new_entry = kzalloc(sizeof(struct st_susfs_sus_net_unix_hlist), GFP_KERNEL);
	if (!new_entry) {
		info.err = -ENOMEM;
		goto out_copy_to_user;
	}
	memcpy(&new_entry->info, &info, sizeof(struct st_susfs_sus_net_unix));
	INIT_LIST_HEAD(&new_entry->list);
	spin_lock(&susfs_spin_lock_sus_net_unix);
	list_add_tail(&new_entry->list, &LH_SUS_NET_UNIX);
	spin_unlock(&susfs_spin_lock_sus_net_unix);
	info.err = 0;
	SUSFS_LOGI("added sus_net_unix uid: %u, inode: %lu\n", info.target_uid, info.inode);
out_copy_to_user:
	if (copy_to_user(&((struct st_susfs_sus_net_unix __user*)*user_info)->err, &info.err, sizeof(info.err))) {
		info.err = -EFAULT;
	}
}
...
void susfs_del_sus_net_unix(void __user **user_info) {
	struct st_susfs_sus_net_unix info = {0};
	struct st_susfs_sus_net_unix_hlist *cursor = NULL, *temp = NULL;
...
bool susfs_is_sus_net_unix(unsigned int uid, unsigned long inode) {
	struct st_susfs_sus_net_unix_hlist *cursor = NULL;
	bool found = false;

	spin_lock(&susfs_spin_lock_sus_net_unix);
	list_for_each_entry(cursor, &LH_SUS_NET_UNIX, list) {
		if (cursor->info.target_uid == uid && cursor->info.inode == inode) {
			found = true;
			break;
		}
	}
...
/*
 * The upstream SuSFS fsnotify "sdcard monitor" (which flipped the flag below
 * to true once /data/media/0/Android became visible after FBE decryption) was
 * never functional on this non-GKI tree: it had no caller and its filename
 * match was broken, so the flag stayed false for the whole boot on the only
 * boot-tested device. Rather than enable an untested code path, the monitor is
 * removed and the flag is left permanently false -- clone_mnt() then always
 * takes the conservative ksu-domain mount check, which is what was validated
 * on-device.
 */
