			  ...
			  void __user **arg)
{
    ...
    if (magic2 == SUSFS_MAGIC && current_uid().val == 0) {
        ...
        if (cmd == CMD_SUSFS_ADD_SUS_NET_PORT) {
            susfs_add_sus_net_port(arg);
            return 0;
        }
        if (cmd == CMD_SUSFS_DEL_SUS_NET_PORT) {
            susfs_del_sus_net_port(arg);
            return 0;
        }
        if (cmd == CMD_SUSFS_CLEAR_SUS_NET_PORT) {
            susfs_clear_sus_net_port(arg);
            return 0;
        }
        if (cmd == CMD_SUSFS_ADD_SUS_NET_UNIX) {
            susfs_add_sus_net_unix(arg);
            return 0;
        }
        if (cmd == CMD_SUSFS_DEL_SUS_NET_UNIX) {
            susfs_del_sus_net_unix(arg);
            return 0;
        }
        if (cmd == CMD_SUSFS_CLEAR_SUS_NET_UNIX) {
